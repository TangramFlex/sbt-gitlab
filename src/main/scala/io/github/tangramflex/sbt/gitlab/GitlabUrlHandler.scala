...
case class GitlabUrlHandler(gitlabHost: String, credentials: GitlabCredentials)
  ...
  private def getHeadersAsDebugString(
  ...
  override def download(
    ...
    try {
      ...
      srcConn
        .setRequestProperty("User-Agent", "Apache Ivy/" + Ivy.getIvyVersion)
      srcConn.setRequestProperty("Accept-Encoding", "gzip,deflate")
      srcConn.setRequestProperty(
        "Accept",
      ...
      if (srcConn.isInstanceOf[HttpURLConnection]) {
        ...
        if (status == 302 || status == 301) {
          ...
          disconnect(srcConn)
          return
        } else if (!checkStatusCode(src, httpCon))
          throw new IOException(
            "The HTTP response code for " + src + " did not indicate a success." + " See log for more detail."
          )
      }
      // do the download
      val inStream = getDecodingInputStream(
        srcConn.getContentEncoding,
      ...
      if (srcConn.getContentEncoding == null) {
        ...
        if (contentLength != -1 && dest.length != contentLength) {
          dest.delete
          throw new IOException(
            "Downloaded file size doesn't match expected Content Length for " + src + ". Please retry."
          )
        }
      }
  ...
  override def upload(
    ...
    try {
      ...
      Message.debug(
        "Request Headers:" + getHeadersAsDebugString(conn.getRequestProperties)
      )
      ...
      catch { case e: IOException => Unit /* ignored */ }
      ...
      if (errorStream != null)
        extra = "; Response Body: " + readTruncated(
          errorStream,
          ERROR_BODY_TRUNCATE_LEN,
          conn.getContentType,
          conn.getContentEncoding
        )
      else if (responseStream != null) {
        val decodingStream =
          getDecodingInputStream(conn.getContentEncoding, responseStream)
        extra = "; Response Body: " + readTruncated(
          responseStream,
          ERROR_BODY_TRUNCATE_LEN,
          conn.getContentType,
      ...
      Message.debug(
        "Response Headers:" + getHeadersAsDebugString(conn.getHeaderFields)
      )
